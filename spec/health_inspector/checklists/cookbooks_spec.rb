...
describe HealthInspector::Checklists::Cookbooks do
  ...
  describe '#server_items' do
    ...
    it 'returns a list of roles from the chef server' do
      health_inspector_context.stub_chain(:rest, :get_rest).with('/cookbooks')
        .and_return roles
      expect(checklist.server_items)
        .to eq('cookbook_one' => Chef::Version.new('1.0.0'),
