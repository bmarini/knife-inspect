...
describe HealthInspector::Checklists::DataBagItems do
  ...
  describe '#server_items' do
    ...
    it 'returns a list of data bag items from the chef server' do
      expect(Chef::DataBag).to receive(:list).and_return data_bags
      { 'data_bag_one' => 'one',
        'data_bag_two' => 'two',
  ...
  describe '#load_item(name)' do
    ...
    it 'instanciates a DataBagItem pairing for that item' do
      expect(checklist).to receive(:load_item_from_local).with(name)
        .and_return name
      expect(checklist).to receive(:load_item_from_server).with(name)
        .and_return name
